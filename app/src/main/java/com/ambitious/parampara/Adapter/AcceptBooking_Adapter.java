...
public class AcceptBooking_Adapter extends RecyclerView.Adapter<AcceptBooking_Adapter.ViewHolder> {
    private Activity activity;
    private List<AcceptedBooking_Result> task;
    ...
    public AcceptBooking_Adapter(Activity activity, List<AcceptedBooking_Result> task, int flag) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        priceaa = task.get(position).getPrice();
        puja = task.get(position).getPoojaName();
        date = task.get(position).getDate();
        time = task.get(position).getTime();
        status = task.get(position).getStatus();
        bookingDate = task.get(position).getEntrydt();
        bookingTime = task.get(position).getEntrydt();
        experiance = task.get(position).getExperience();

        //  convePrice = task.get(position).get();

        address = task.get(position).getFlat() + " , " + task.get(position).getColony() + " , " + task.get(position).getLandmark();
        ...
        dt = task.get(position).getPrice();
        id = task.get(position).getPoojaBookingId();
...
//        } else {
        ...
        if (task.get(position).getStatus().equalsIgnoreCase("completed") && Integer.parseInt(task.get(position).getUser_rating()) <= 0) {
        ...
        try {
            if (!task.get(position).getAvg_rating().isEmpty()) {
                ...
                avg = task.get(position).getAvg_rating();
            } else {
                avg = "0";
            }
        } catch (Exception e) {
    ...
    private void addRate(final int rate, final int pos) {
        ...
        Call<ResponseBody> call = AppConfig.loadInterface().addPoojaRating("" + task.get(pos).getPoojaBookingId(), "" + rate, "" + task.get(pos).getUserId(), "" + feedback_edt.getText().toString(), "user");
                ...
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        if (error.equals("200")) {
                            ...
                            task.get(pos).setUser_rating("" + rate);
